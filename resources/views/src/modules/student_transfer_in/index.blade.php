                ...
                <table id="example" class="table table-striped">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td
                                class="student-code"
                                data-student-code="{{ $result->student_code }}"
                            >
                                {{ $result->student_code }}
                            </td>
                            @if(optional($user->roles()->first())->name !== 'HOI Primary')
                            <td>
                                {{ $result->schoolInfo->school_name ?? 'N/A' }}
                            </td>
                            @endif
                            <td>
                                {{ $result->studentInfo->getAttributes()['studentname'] ?? 'N/A' }}
                            </td>
                            <td>{{ $result->studentInfo->dob ?? 'N/A' }}</td>
                            <td>
                                {{ $result->studentInfo->guardian_name ?? 'N/A' }}
                            </td>
                            <td class="text-center">
                                {{ $result->studentInfo->cur_roll_number ?? 'N/A' }}
                            </td>
                            <td>{{ $result->reasonInfo->name ?? 'N/A' }}</td>
...
<script>
    $(document).ready(function () {
      ...
      /* Fetch reasons only once */
        ...
        $(document).on('click', '#btn_transfer_out', function () {
            ...
            const reason_code_fk = $("#transfer_out_reason").val();
            ...
            showAlert({
                ...
                message: 'Do you want to transfer out this student?',
                confirmText: 'Send'
            }).then(confirmed => {
                ...
                sendRequest(
                    "{{ route('student.transferout.store') }}",
                    "POST",
                    null,
                    {
